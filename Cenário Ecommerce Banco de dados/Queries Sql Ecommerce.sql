...
-- Vendedores que também são fornecedores (Consulta aninhada)
select v.RazaoSocial from vendedor v 
where v.RazaoSocial in 
(select f.RazaoSocial from fornecedor f);

-- Vendedores que também são fornecedores (usando inner join)

select v.RazaoSocial from vendedor v inner join fornecedor f on f.RazaoSocial=v.RazaoSocial; 